...
describe('AddonDetail', () => {
  ...
  it('disables install switch and uses button for OpenSearch plugins', () => {
    const AddSearchProviderSpy = sinon.stub(window.external,
      'AddSearchProvider');
    const rootNode = renderAsDOMNode({
    ...
    assert.ok(AddSearchProviderSpy.calledWith(fakeAddon.installURL));
  });
  ...
  it('disables the button/does nothing if OpenSearch is not available', () => {
    ...
    assert.notOk(AddSearchProviderSpy.called);
  });
